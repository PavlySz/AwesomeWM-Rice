...
scripts = {"booking": "etl/src/booking_report.sql", "flight": "etl/src/flight_report.sql"}
sql_exec = {}
...
while (
    redshift.describe_clusters(ClusterIdentifier=bright_config["RS_ENDPOINT"])["Clusters"][0][
        "ClusterStatus"
    ]
    != "available"
):
...
# ! redshift_connector.error.InterfaceError: ('communication error',
#   TimeoutError(110, 'Connection timed out'))
# TODO Catch exception where connection times out when ran locally
...
try:
    ...
    logger.error("Could not connect to %s.", bright_config['RS_HOST'])
    logger.error("Make sure you are running the script \
                 from the server and not from a local machine.")
    sys.exit(1)
...
for query_to_be_executed in sql_queries_to_be_executed:
    ...
    for command_number, command in enumerate(commands):
        # ? This was probably made just for debugging. I am going to leave it be just in case.
        if len(command) < 10:
            continue
