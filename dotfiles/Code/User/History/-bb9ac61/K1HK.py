...
pgconn = psycopg2.connect(
    dbname="flo_stage_us", user="flo", password="pvHZ&5y3A&nT8TQN", host="172.20.20.90"
)
pgcur = pgconn.cursor()
...
# 'rs-ds-prod.cebwpov4juzc.us-west-1.redshift.amazonaws.com'
rs_host = "54.177.154.150"
rs_database = "dev"
rs_user = "ds"
rs_password = "x#prdsATrsdsjsz1212"

# Connects to Redshift cluster using AWS credentials
rsconn = redshift_connector.connect(
    host=rs_host, database=rs_database, user=rs_user, password=rs_password
)
...
# and indepth!='booking-by-earlydep'"
read_template = """
...
for t in test_instances:
    if t[2] in test_params:
        test_params[t[2]].append(
            {"param1": t[3], "param2": t[4], "param3": t[5], "id": t[0]})
    else:
        test_params[t[2]] = []
        test_params[t[2]].append(
            {"param1": t[3], "param2": t[4], "param3": t[5], "id": t[0]})
...
for r in rs_template.keys():  # r is the report identifier
    ...
    for test_id in rs_results[r].keys():
        ...
        for f in range(len(rs_results[r][test_id].columns)):  # f as field
            ...
            if (
                abs(
                    rs_results[r][test_id].iloc[0, [f]].values
                    - pg_results[r][test_id].iloc[0, [f]].values
                )
        ...
        print(
            "QA outcome - ", r, ": id ", test_id, " check/failed: ", str(
                chkcnt), "/", str(errcnt)
        )

print("QA outcome - Final: check/failed: ",
      str(totalchkcnt), "/", str(totalerrcnt))
# pdb.set_trace()
